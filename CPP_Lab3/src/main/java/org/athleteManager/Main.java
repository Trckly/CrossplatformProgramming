...
public class Main extends Application {
    ...
    private void PlainFileOut(Athlete athlete) {
        try(BufferedOutputStream bos = new BufferedOutputStream(new FileOutputStream("athlete_data.txt", true))){
            ObjectOutputStream oos = new ObjectOutputStream(bos);
            oos.writeChars(athlete.getFirstName());
            oos.writeChars(athlete.getLastName());
            oos.writeChars(athlete.getSport());
        } catch (Exception e) {
            System.err.println(e.getMessage());
        }

        try(FileOutputStream fos = new FileOutputStream("athlete_data.txt", true)){
            ObjectOutputStream oos = new ObjectOutputStream(fos);
            oos.writeInt(athlete.getMedalsCount());
        } catch (Exception e) {
            System.err.println(e.getMessage());
        }
    }

    private List<Athlete> PlainFileIn() {
        List<Athlete> athletes = new ArrayList<>();
        try(BufferedInputStream bis = new BufferedInputStream(new FileInputStream("athlete_data.txt"))){
            ObjectInputStream ois = new ObjectInputStream(bis);
            while (true) {
                try {
                    Object ath = ois.readObject();
                    Athlete newAthlete = (Athlete) ath;
                    if(newAthlete == null){
                        System.out.println("Null Athlete");
                    }
                    else {
